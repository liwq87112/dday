...
@interface ZPLoopView () <UICollectionViewDelegate, UICollectionViewDataSource>
...
@implementation ZPLoopView

- (instancetype)initWithImageArray:(NSArray *)imageArray {
    if (self = [super init]) {
        UICollectionView *collectionView = [[UICollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:[[ZPLoopViewLayout alloc] init]];
        [collectionView registerClass:[ZPLoopViewCell class] forCellWithReuseIdentifier:ID];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ZPLoopViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:ID forIndexPath:indexPath];
...
- (void)addTimer {
    ...
    self.timer = [ZPWeakTimerTargetObject scheduledTimerWithTimeInterval:1.5 target:self selector:@selector(nextImage) userInfo:nil repeats:YES];
